...
$destinos = $db->query("SELECT id, destino FROM destinos ORDER BY destino")->fetchAll();
                ...
                <div class="form-group col-md-3">
                  ...
                  <select name="id_destino" class="form-control" required>
                    <?php foreach($destinos as $d):?>
                      <option value="<?=$d['id']?>" <?=($viaje['id_destino'] ?? '')==$d['id']?'selected':''?>><?=htmlspecialchars($d['destino'])?></option>
                    <?php endforeach;?>
                  </select>
                </div>
