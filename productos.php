  ...
  <body>
    ...
    <script type="text/javascript">
        ...
        searchField.on('keydown', function(e) {
          ...
          if (e.key === 'Enter' && noResultsShown) {
            ...
            $.ajax({
              ...
              type: "POST",
              datatype:"json",
              data:  {accion: "addpresentacion", nombre: nombre},
              success: function(data) {
                if(data=="1"){
                  console.log("Se agrego correctamente: " + searchTerm);
                  // Agregamos la nueva opción al select
                  let newOption = new Option(nombre, nombre, false, false);
                  $('#id_presentacion').append(newOption).trigger('change');

                  // Seleccionamos la nueva opción y enfocamos el select
                  $('#id_presentacion').val(nombre).trigger('change');
                  $('#id_presentacion').select2('close');
                }else{
                  swal({
                    icon: 'error',
      ...
      $('#formAdmin').submit(function(e){
        ...
        console.log("id_presentacion: " + id_presentacion + " , id_unidad_medida: " + id_unidad_medida)
